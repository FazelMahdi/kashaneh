...
export default function AddDriverDialog({ show, onClose, onUpdate, driver }) {
  ...
  const onSubmit = async (data) => {
    ...
    if (driver) {
      http
        .put(`/api/v1/driver/${driver.id}/update`, {
          ...data,
          fullPelak: `${data.pelak.p1}${data.pelak.p2}${data.pelak.p3}`,
        })
        .then(() => {
          setOpen(false);
          onClose(false);
      ...
      <DialogContent dividers>
        <form className="flex flex-col justify-center items-center">
          ...
          <div className="flex justify-between flex-wrap mt-5">
            <div className="flex justify-between border border-solid border-gray-800 rounded-sm overflow-hidden bg-yellow-500">
